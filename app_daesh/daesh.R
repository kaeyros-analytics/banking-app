  ...
  dashboardBody(
    tabsetPanel(
      tabPanel(tags$b("General"),
               ...
               fluidRow(
                 box(width = 6, highchartOutput("totalCustomersByGender")),
                 box(width = 6, highchartOutput("totalCustomersByMarital"))
                 ),
               fluidRow(
                 box(width = 6, highchartOutput("CustomersStatusByAgency")),
                 box(width = 6, highchartOutput("CustomersStatusByActivity"))
                 )
               ),
      tabPanel(tags$b("Inactive"),
               fluidRow(
                 box(width = 6, highchartOutput("CustomersStatusByBalanceClass")),
                 box(width = 6, highchartOutput("CustomersStatusByAgeClass"))
                 ),
               fluidRow(
                 box(width = 12, highchartOutput("TemporelSerie"))
                 )
               ),
      tabPanel(tags$b("Localisation"),
               fluidRow(
                 box(width = 12, leafletOutput("Carte"))
                 ),
               fluidRow(
                 downloadButton("downloadCSV", tags$b("Download Table")),
                 box(width = 12, tags$h3(tags$b("List of Inactive Accounts")), rHandsontableOutput('Table'))
                 )
               ),
      tabPanel(tags$b("Prediction"),
               fluidRow(
                 box(width = 12,
                     box(width = 2, selectInput("age", "Class Age",choices = unique(account$age_interval))),
                     box(width = 2, selectInput("gender", "Gender",choices = unique(account$gender))),
                     box(width = 2, selectInput("occupation", "Occupation",choices = unique(account$occupation))),
                     box(width = 2, selectInput("revenue", "Monthly Revenue",choices = unique(account$monthly_revenue_class))),
                     box(width = 2, sliderInput("children", "Number of Children",min = 0, max = 10, value = 0, step = 1)),
                     box(width = 2, selectInput("current_balance", "Current Balance",choices = unique(agios$balance_after_agios_class)))
                     )
                 ),
               fluidRow(
                 box(width = 12,
                     box(width = 2, actionButton("btn", "Cliquez ici")),
                     box(width = 2, actionButton("bt", "Cliquez ici")))
                 )
               )
    )
  )
...
server <- function(input, output) {
  ...
  leaflet(df) %>%
    ...
    output$Table <- renderRHandsontable({
      ...
      rhandsontable(data[, !colnames(data) %in% c("Year Agios","Status")]) %>% 
